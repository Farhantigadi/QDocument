      ...
      {document && <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section>
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
            <div><p className="eyebrow text-accent">{document.category} / {document.sourceType === 'link' ? 'Google Drive link' : (document.fileType ?? 'file').toUpperCase()}</p><h1 className="display mt-2 max-w-2xl text-4xl leading-tight sm:text-5xl" data-testid="heading-document-title">{document.title}</h1><p className="mt-3 text-sm text-muted-foreground">Updated {formatDate(document.updatedAt, true)}</p></div>
          ...
          <div className="paper-grid mt-8 flex min-h-[390px] items-center justify-center overflow-hidden rounded-2xl border border-border bg-card p-5 sm:min-h-[510px]">
            {document.sourceType === 'link' && document.sourceUrl ? (
              ...
              <iframe
                ...
                className="h-[470px] w-full rounded-lg border border-border bg-background"
              ...
              <img
                ...
                className="max-h-[470px] max-w-full rounded-lg object-contain shadow-[var(--shadow-lg)]"
              ...
              <div className="w-full max-w-sm rounded-xl border border-border bg-background p-7 text-center shadow-[var(--shadow-sm)]" data-testid="empty-document-preview">
                ...
                <p className="mt-5 font-semibold">Private vault file</p>
                <p className="mt-2 text-sm leading-6 text-muted-foreground">This file is stored privately and can only be opened from your signed-in vault.</p>
              </div>
            )}
          </div>
