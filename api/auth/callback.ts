...
// Google OAuth is deferred — coming soon
export default function handler(_req: VercelRequest, res: VercelResponse) {
  res.status(404).json({ error: "Google OAuth is not enabled yet." });
}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const token = await signSession({ sub: profile.id!, name: profile.name ?? "", email, role });
